...
public class HadronRecipes extends SerializableRecipe {
	...
	/*
	 * Since we're dealing with like 10 or so recipes, using a HashMap (or to
	 * combine two keys, a HashMap *in* a HashMap)
	 * would be less performant than those few steps through a good old Array list,
	 * and it's much easier to implement too.
	 */
	private static final List<HadronRecipe> recipes = new ArrayList();

	/*
	 * We CAN actually implement recipes with the same input items but different
	 * momentum requirements.
	 * Just be sure to register the higher requirement BEFORE the lower one since
	 * those need to be checked first.
	 * 
	 * It's important to remember that, ok?
	 * 
	 * Update, T+6 minutes: I went for coffee and already forgot what I was doing,
	 * good thing I keep these notes, hehe.
	 * Having multiple recipes with different momentum requirements (at most I would
	 * expect 2) isn't exactly necessary
	 * since the thing differentiates between ring and line accelerator mode, and
	 * line accelerators are by design always shorter anyway.
	 */
	@Override
	 ...
	 * @return either null (no recipe) or an ItemStack array with 2 non-null
	 *         instances
	 */
	public static ItemStack[] getOutput(ItemStack in1, ItemStack in2, int momentum, boolean analysisOnly) {
		...
		for (HadronRecipe r : recipes) {
			...
			if ((r.in1.isApplicable(in1) && r.in2.isApplicable(in2)) ||
				...
				if (analysisOnly && !r.analysisOnly)
					returnCode = EnumHadronState.NORESULT_WRONG_MODE;
				if (momentum < r.momentum)
					returnCode = EnumHadronState.NORESULT_TOO_SLOW;
	...
	public static class HadronRecipe {
		...
		public HadronRecipe(ItemStack in1, ItemStack in2, int momentum, ItemStack out1, ItemStack out2,
